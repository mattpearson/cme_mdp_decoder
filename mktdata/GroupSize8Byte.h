...
#ifndef _MKTDATA_GROUPSIZE8BYTE_H_
#define _MKTDATA_GROUPSIZE8BYTE_H_
...
public:
    GroupSize8Byte(void) : m_buffer(nullptr), m_offset(0) {}

    GroupSize8Byte(char *buffer, const std::uint64_t bufferLength, const std::uint64_t actingVersion)
    {
    ...
    GroupSize8Byte &wrap(char *buffer, const std::uint64_t offset, const std::uint64_t actingVersion, const std::uint64_t bufferLength)
    {
    ...
    GroupSize8Byte &blockLength(const std::uint16_t value)
    {
    ...
    {
        return (*((std::uint8_t *)(m_buffer + m_offset + 7)));
    }

    GroupSize8Byte &numInGroup(const std::uint8_t value)
    {
