...
#ifndef _MKTDATA_PRICENULL_H_
#define _MKTDATA_PRICENULL_H_
...
public:
    PRICENULL(void) : m_buffer(nullptr), m_offset(0) {}

    PRICENULL(char *buffer, const std::uint64_t bufferLength, const std::uint64_t actingVersion)
    {
    ...
    PRICENULL &wrap(char *buffer, const std::uint64_t offset, const std::uint64_t actingVersion, const std::uint64_t bufferLength)
    {
    ...
    PRICENULL &mantissa(const std::int64_t value)
    {
